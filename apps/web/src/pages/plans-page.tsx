...
import { useCreatePlan, useDeactivatePlan, usePlans, useUpdatePlan } from "@/hooks/use-workouts";
...
export function PlansPage() {
  ...
  const setRowState = (planId: string, updates: Partial<RowState>) => {
    setRowStates((prev) => ({
      ...prev,
      [planId]: {
        ...(prev[planId] ?? { isSaving: false, isDeactivating: false, isSuccess: false }),
        ...updates
      }
    }));
  };
    ...
    <div className="space-y-[var(--section-gap)] transition-colors duration-300">
      <header className="apple-card shadow-xl p-8 border border-[var(--border-base)]">
        ...
        <p className="mt-3 max-w-3xl text-sm md:text-base leading-relaxed text-[var(--text-secondary)]">
          Define las reglas de membresía, límites de créditos y pases libres. Estos planes se
          aplican a las nuevas suscripciones de miembros.
        </p>
      </header>

      <section className="grid gap-8 xl:grid-cols-[1fr_1.2fr]">
        <div className="apple-card shadow-lg p-8 h-fit">
          <div>
          ...
          <form className="mt-8 space-y-5" onSubmit={handleCreatePlan}>
            ...
            <div className="grid gap-4 pt-2">
              <label className="flex items-center gap-3 rounded-2xl bg-[var(--bg-surface-secondary)] p-4 text-sm font-bold text-[var(--text-primary)] cursor-pointer hover:bg-[var(--accent-soft)] transition-colors group border border-transparent hover:border-[var(--accent-soft)]">
                <input
                  name="allows_free_pass"
                  type="checkbox"
                  className="h-5 w-5 rounded-lg accent-[var(--accent)]"
                />
            ...
            plans.map((plan) => {
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <div
                          className={`h-2 w-2 rounded-full ${plan.active ? "bg-[var(--success)] animate-pulse" : "bg-[var(--text-muted)]"}`}
                        />
                  ...
                  <form className="mt-10 space-y-6" onSubmit={handleUpdatePlan(plan.id)}>
            ...
            <div className="apple-card border-dashed border-2 flex flex-col items-center justify-center py-20 text-center space-y-4 opacity-70">
              ...
              <div>
                ...
                <p className="text-sm text-[var(--text-muted)] max-w-xs mt-1">
                  Empieza creando el primer plan de membresía para tu gimnasio.
                </p>
              </div>
            </div>
          ) : null}
